...
function StatPill({label, value}: {label: string; value: string}) {
  return (
    <div className="rounded-full border border-gotham-line bg-gotham-panel px-3 py-1 text-[11px] uppercase tracking-[0.28em] text-gotham-muted">
...
export default function SpectrumAnalyzer(props: SpectrumAnalyzerProps) {
  ...
  return (
    <div className="h-screen overflow-hidden bg-gotham-shell text-gotham-text">
      <div className="relative h-full overflow-hidden px-4 py-4 sm:px-6 sm:py-6">
        ...
        <div className="relative mx-auto flex h-full w-full max-w-[1500px] flex-col rounded-[2rem] border border-gotham-line bg-gotham-window/92 p-4 shadow-[0_30px_120px_rgba(0,0,0,0.55)] backdrop-blur-xl sm:p-6">
          ...
          <main className="mt-4 flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
            {props.error ? <ErrorBanner message={props.error.message} /> : null}

            <section className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-[1.75rem] border border-gotham-line bg-gotham-panel/88 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:p-5">
              ...
              <div className="relative mt-0 flex min-h-0 flex-1 flex-col gap-0 overflow-hidden lg:flex-row">
                <SpectrumGraph
                  bars={props.bars}
                  sampleRate={sampleRate}
                  fftSize={fftSize}
                  className="relative min-h-[320px] flex-1 overflow-hidden rounded-[1.25rem] border border-gotham-line/70 bg-[linear-gradient(180deg,rgba(5,11,20,0.82)_0%,rgba(8,15,27,0.95)_100%)]"
                />

                <VolumeMeter
                  level={meterLevel}
                  peak={meterPeak}
                  className="h-[220px] w-full lg:h-auto lg:w-[132px] lg:ml-0"
                />
              </div>
            </section>
          </main>
